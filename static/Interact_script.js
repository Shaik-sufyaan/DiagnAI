...
const chat = []; //Array to store chat history


// Add a click event listener to the delete button
deleteButton.addEventListener('click', () => {
    if (chatList) {
        chatList.innerHTML = ''; // Clear all chat messages
        chat = []; // Optional: Reset chat array if you maintain it...we can keep anyone method to clear the chats
        console.log("Chat history cleared.");
    } else {
        console.warn("No chat container found to clear!");
    }
});
...
const generateAPIResponse = async (incomingMessageDiv) => {
    ...
    try {
        ...
        if (data.candidates && data.candidates[0] && data.candidates[0].content) {
            ...
            // Add the API response to the chat array
            chat.push({ role: "bot", message: apiResponse });
        } else {
...
const handleOutgoingChat = () => {
    ...
    chat.push({ role: "user", message: userMessage });
...
// // Select the delete button and the input field
// const deleteButton = document.getElementById('delete-button');
// const AllResponses = document.getElementById(Chat);
